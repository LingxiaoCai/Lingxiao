...
def morse_translator(text):
    ...
    text = text.upper() #change into upper letters

    new_text = ''
    for words in text.split(): #split into words and create a list
        new_text = new_text + words + '/' #create new string with / spliting words

    final = ''
    for letters in new_text:
        if letters in morse_code_dict:
            letters = morse_code_dict[letters] #change into morse code
        final = final + letters + ' ' #create new string with space spliting letters
    final = final[0:-2]
    return(final)
