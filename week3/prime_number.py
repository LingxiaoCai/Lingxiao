...
def is_prime(number):
    ...
    #for any int greater than 2, start a for loop from 2 to number-1
    elif number > 2:
        for d in range(2,number):
            #return False when there is a positive divisor
            if number % d == 0:
                return False
            #continue the loop when there is no positive divisor until number-2
            elif number % d != 0 and d != number - 1:
                continue
            #return True when no positive divitor until number-1
            else:
                return True   
            
    #return False for any int smaller than 2
    else:
        return False
...
print(is_prime(12324647))  # Expected output: True
print(is_prime(123456789))  # Expected output: False
