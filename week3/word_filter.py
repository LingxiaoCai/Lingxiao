...
def word_filter_counter(text, filter_words):
    ...
    #Translate letters into lower letters
    text = text.lower()  
    
    #Translate punctuation into space
    transpunct = str.maketrans({key:" " for key in punctuation})
    text_new = text.translate(transpunct)
    
    #Split the words and create a list of words
    text_final = text_new.split()
    
    #Count words in filter_words
    for words in text_final:  #loop through word list
        if words in filter_words:  #determine whether in the filter_words list
            if words not in word_count:  
                word_count[words] = 1  #add into dictionary
            else:
                word_count[words] += 1  #add 1 in existing value
    return(word_count)
